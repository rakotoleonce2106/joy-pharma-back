...
try {
    ...
    // The package should be installed via composer: dotenv-php/dotenv-vault
    if (file_exists(__DIR__ . '/vendor/autoload.php')) {
        require_once __DIR__ . '/vendor/autoload.php';
    }
    
    // Check if the dotenv-vault package is available
    if (class_exists('\Dotenv\Vault\Vault')) {
        // Use the official dotenv-vault PHP package
        \Dotenv\Vault\Vault::load($vaultFile);
        error_log("Loading env from encrypted .env.vault");
        ...
        error_log("dotenv-vault package not found. Install it with: composer require dotenv-php/dotenv-vault");
    }
} catch (\Exception $e) {
